...
void MakeUpABookmarkName(char *dst, size_t siz, char *src)
{
	...
	token = str;
#if MSRIPv6
    /* Check for an IPv6 address first, as code below doesn't grok them.
     * Use inet6_addr as it's easier than duplicating parser code here.
     */
    if (inet6_addr(token, &dummy))
        token = "misc";
    else
#endif
