...
/*
 * We need to pull in the manifest information so that user dlls's which
 * do not include any headers get a manifest
 */
#if defined (_M_IX86)
        #pragma comment(linker, "/include:__forceCRTManifest")
#else  /* defined (_M_IX86) */
        #pragma comment(linker, "/include:_forceCRTManifest")
