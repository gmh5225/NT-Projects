...
extern "C" void __clrcall _exit_callback(void)
{
    ...
    if ((onexitbegin_m != (_CPVFV *)-1) && (onexitbegin_m != NULL) && (onexitend_m != NULL)) {
        while ( --onexitend_m >= onexitbegin_m )
        {
            /*
             * if current table entry is non-NULL,
             * call thru it.
             */
            if ( *onexitend_m != _encoded_null())
                (*(_CPVFV)_decode_pointer(*onexitend_m))();
        }
        /*
...
extern "C" int __clrcall _atexit_m_appdomain(_CPVFV func)
{
    ...
    int retval = _atexit_helper((_CPVFV)_encode_pointer(func), &__exit_list_size_app_domain, &onexitend_app_domain, &onexitbegin_app_domain);

    __onexitbegin_app_domain = (_CPVFV *)_encode_pointer(onexitbegin_app_domain);
    __onexitend_app_domain   = (_CPVFV *)_encode_pointer(onexitend_app_domain);
