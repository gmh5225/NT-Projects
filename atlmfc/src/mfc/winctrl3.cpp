...
inline HMODULE AfxLoadSystemLibraryUsingFullPath(const TCHAR *pszLibrary)
{
	TCHAR szLoadPath[MAX_PATH+1];
	if (::GetSystemDirectory(szLoadPath, _countof(szLoadPath)) == 0)
	{
		return NULL;
	}

	if (szLoadPath[_tcslen(szLoadPath)-1] != _T('\\'))
	{
		if (_tcscat_s(szLoadPath, _countof(szLoadPath), _T("\\")) != 0)
		{
			return NULL;
		}
	}

	if (_tcscat_s(szLoadPath, _countof(szLoadPath), pszLibrary) != 0)
	{
		return NULL;
	}

	return(::LoadLibrary(szLoadPath));
}
...
struct _ThemeHelper
{
	...
	{
		static HMODULE hThemeDll = AfxLoadSystemLibraryUsingFullPath(_T("UxTheme.dll"));
