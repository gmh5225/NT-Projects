...
console.println("Acrobat Forms Built-in Functions Version 4.0");
...
trans = new Object();
trans.blindsH		= "Blinds Horizontal";
trans.blindsV		= "Blinds Vertical";
trans.boxI			= "Box In";
trans.boxO			= "Box Out";
trans.dissolve		= "Dissolve";
trans.glitterD		= "Glitter Down";
trans.glitterR		= "Glitter Right";
trans.glitterRD		= "Glitter Right-Down";
trans.none			= "No Transition";
trans.random		= "Random Transition";
trans.replace		= "Replace";
trans.splitHI		= "Split Horizontal In";
trans.splitHO		= "Split Horizontal Out";
trans.splitVI		= "Split Vertical In";
trans.splitVO		= "Split Vertical Out";
trans.wipeD			= "Wipe Down";
trans.wipeL			= "Wipe Left";
trans.wipeR			= "Wipe Right";
trans.wipeU			= "Wipe Up";
...
{       /* This function validates the current keystroke event to make sure the
	...
	{
		if (event.willCommit)
			app.alert(IDS_INVALID_VALUE + " [ " + event.target.name + " ]", 0);
		else
			app.beep(0);
		event.rc = false;
	}
...
{	/* This function validates the current keystroke event to make sure the
	** key pressed is reasonable for a date field. */
	if(event.willCommit && !AFParseDateEx(AFMergeChange(event), cFormat)) {
		...
		if (event.willCommit)
			app.alert(IDS_INVALID_DATE + " [ " + event.target.name + " ]", 0);
		else
			app.beep(0);
		event.rc = false;
	}
